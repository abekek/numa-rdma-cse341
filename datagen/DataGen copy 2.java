    ...
    public static void main(String[] args) throws SQLException, IOException, java.lang.ClassNotFoundException{
        ...
        try (
            ...
            while((line = br.readLine()) != null){
                ...
                String queryInsertLease = String.format("INSERT INTO LEASE VALUES ('%s', '%s', '%s', '%s', '', '', '%s')", attributes[0], tenantIds.get(rowNum), attributes[1], attributes[2], Double.toString(paymentAmount));
                s.executeUpdate(queryInsertLease);

                String queryInsertPayment = String.format("INSERT INTO PAYMENT VALUES('%s', '%s', '%s', '%s')", attributes[3], Double.toString(paymentAmount), paymentTypes[(int)(Math.random()*3)], attributes[0]);
